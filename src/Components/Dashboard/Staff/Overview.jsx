...
function Overview() {
  ...
  // Only access top and bottom agents if overviewData is available
  const topAgentsRaw = overviewData[0] || [];  // Default to empty array if data is unavailable
  const bottomAgentsRaw = overviewData[1] || [];  // Default to empty array if data is unavailable

  const topThree = topAgentsRaw
    ?.slice(0, 3)
    ?.map(agent => ({
  ...
  const bottomThree = bottomAgentsRaw
    ?.slice(0, 3)
    ?.map(agent => ({
      name: agent.agentName,
      amount: Number(agent.totalKg),
  ...
  return (
    <div className="px-[30px] py-[40px] w-full">
      <div className="flex flex-col">
        ...
        {isLoading ? (
          <div className="text-center"><Spinner size='md'/></div> // Loading message or spinner
        ) : (
